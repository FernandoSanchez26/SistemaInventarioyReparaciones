...
namespace SistemaInventarioyReparaciones.Areas.Identity.Pages.Account
{
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!await _roleManager.RoleExistsAsync(DS.Role_Jefatura))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(DS.Role_Jefatura));
                    }
                    if (!await _roleManager.RoleExistsAsync(DS.Role_Consulta))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(DS.Role_Consulta));
                    }
                    if (!await _roleManager.RoleExistsAsync(DS.Role_Adm_Inventario))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(DS.Role_Adm_Inventario));
                    }

                    //await _userManager.AddToRoleAsync(user, DS.Role_Jefatura);
