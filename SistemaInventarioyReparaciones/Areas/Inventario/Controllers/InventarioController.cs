...
namespace SistemaInventarioyReparaciones.Areas.Inventario.Controllers
{
    ...
    public class InventarioController : Controller
    {
        ...
        public IActionResult NuevoInventario()
        {
            ...
            {
                Inventario = new SistemaInventarioyReparaciones.Modelos.Inventario(),
        ...
        public IActionResult EgresoInventario(int id)
        {
            return View();
        }
        ...
        public async Task<IActionResult> Detalle(int? id)
        {
            ...
            var bodegaProducto = await _unidadTrabajo.BodegaProducto.ObtenerPrimero(b => b.Id == id, incluirPropiedades: "Producto,Producto.Categoria,Producto.Marca");


            if (bodegaProducto == null)
            {
                despachoProductoVM.Stock = 0;
            }
            else
            {
                despachoProductoVM.Producto = bodegaProducto.Producto;
                despachoProductoVM.Stock = bodegaProducto.Cantidad;
            }

            despachoProductoVM.DespachoProducto = new DespachoProducto()
            {
                Producto = despachoProductoVM.Producto,
                ProductoId = despachoProductoVM.Producto.Id
            };
        ...
        [ValidateAntiForgeryToken]
        [Authorize]
        public async Task<IActionResult> Detalle(DespachoProductoVM despachoProductoVM)
        {
            var claimIdentity = (ClaimsIdentity)User.Identity;
            var claim = claimIdentity.FindFirst(ClaimTypes.NameIdentifier);
            despachoProductoVM.DespachoProducto.UsuarioAppId = claim.Value;

            DespachoProducto despachoBD = await _unidadTrabajo.DespachoProducto.ObtenerPrimero(c => c.UsuarioAppId == claim.Value &&
                                                                                      c.ProductoId == despachoProductoVM.DespachoProducto.ProductoId);
            if (despachoBD == null)
            {
                await _unidadTrabajo.DespachoProducto.Agregar(despachoProductoVM.DespachoProducto);
            }
            else
            {
                despachoBD.Cantidad += despachoProductoVM.DespachoProducto.Cantidad;
                _unidadTrabajo.DespachoProducto.Actualizar(despachoBD);
            }
            await _unidadTrabajo.Guardar();
            TempData[DS.Exitosa] = "Producto agregado al despacho de productos";

            // Agregar valor a la Sesion
            var despachoProductoLista = await _unidadTrabajo.DespachoProducto.ObtenerTodos(c => c.UsuarioAppId == claim.Value);
            var numeroProductos = despachoProductoLista.Count();  // Numero de Registros
        ...
        public async Task<IActionResult> ImprimirKardexEs(string fechaInicio, string fechaFinal, int productoId)
        {
            ...
            kardexInventarioVM.FechaInicio = DateTime.Parse(fechaInicio); //  00:00:00
            ...
            return new ViewAsPdf("ImprimirKardexEs", kardexInventarioVM)
            {
        ...
        public async Task<IActionResult> BuscarProducto(string term)
        {
            ...
            {
                var listaProductos = await _unidadTrabajo.Producto.ObtenerTodos(p => p.EstadoP == true);
                var data = listaProductos.Where(x => x.Activo_Serie.Contains(term, StringComparison.OrdinalIgnoreCase) ||
